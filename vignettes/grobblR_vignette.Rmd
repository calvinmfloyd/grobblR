...
Note: To plot and view the grobs as we produce them, we can either use the generic `plot(grob)` or `gridExtra::grid.arrange(grob)`. Since the plots outputted by `grid.arrange()` more resemble the PDF reports outputted by by `grobblR`, we will load in the `gridExtra` package, and use its functionality to display the grobs in this vignette. Also, some parts of grobblR were created with the idea that they would work well with the `%>%` pipe, so we will load that in from the `dplyr` package.
...
It was the goal of this package to create flexible functionality that would allow the user to combine these types of grobs in any way they desired.

***

## Grob Layout

When coming up with what the `grobblR` package would look like, we first had to decide on the most intuitive way to layout the user's grobs on the page. We decided on a Shiny-like bootstrap method, which would incorporate nested grob-rows (a row combining one or several grobs) and grob-columns (a column combining one or several grobs) within an overall layout of grobs (which we will call a grob-layout). What the main grob-layout function does is evaluate the overall dimensions of the page you desire, and then distributes heights and widths to the grobs that will make up the page, based on proportions and the amount of grob-rows (`grob_row()`) and grob-columns (`grob_col()`) within the overall grob-layout (`grob_layout()`). For example, if we wanted to do a simple `2 x 2` grid system on a `100mm x 100mm` page, we would simply write:
...
The `grob_row()`'s tell the `grob_layout()` that we want two rows on the outermost layer, and since there are two `grob_col()`'s within each of the `grob_row()`'s, the result is a `2 x 2` grid. But the result doesn't have to be a nice `2 x 2` grid - we could have a layout where the first row has two columns and the second has one:
...
Similar to the objects outputted by `grob_layout()`, the objects outputted by `grob_row()` and `grob_col()` also can produce their own grobs. Not as clean as performing the actions through `grob_layout()`, but the height and width can also be tweaked for these grob-column and grob-row objects - where the default for both is `100mm x 100mm` (setting `border = TRUE` will be addressed later on).

```{r grob_col grob}
gc = grob_col('grob-column grob')
gc$height = 50
gc$width = 50
grid.arrange(gc$grob)
```

```{r grob_row grob}
gr = grob_row(
  grob_col('grob-column 1', border = TRUE),
  grob_col('grob-column 2', border = TRUE)
  )
gr$height = 50
gr$width = 100
grid.arrange(gr$grob)
```
...
One benefit of this is in case the user isn't totally sure how many grob-rows there will be within the grob-layout, the user does not explicitly need to define each grob-row: He/she can dynamically create the list of grob-rows and insert that list within `grob_layout()`.

```{r list, loose demonstration (grob-col)}

list_of_grob_rows = list()
for(i in 1:4){
  list_of_grob_rows[[i]] = grob_row(border = TRUE, grob_col(i), grob_col(i^2))
}
...
for(i in 1:4){
  list_of_grob_cols[[i]] = grob_col(border = TRUE, i)
}
...
The `padding_p` and `padding` parameters within `grob_layout()` controls the very outside padding on the page, and the `padding_p` and `padding` parameters within `grob_col()` controls the amount of room between the grob itself and the outer grob-row - helping separate the grobs and giving them air to breathe. Note that `padding_p` is the default and takes a small percentage of the height or width (depending on which is less) and uses that as the padding around the space. If `padding` is set to a numeric amount in the units specified in the grob-layout, then that will override `padding_p`.

The padding parameters within `grob_col()` allows grob-columns to be shifted around its allotted space, making use of the `hjust` and `vjust` parameters within `grob_col()`. The defaults for both are `0.5` (having the grob centered in its allotted space). Setting `hjust` to `1.0` pushes the grob as far right as it can go (opposite for setting it to `0.0`), and setting `vjust` to `1.0` pushes the grob as far to the top as it can go (again, opposite for setting it to `0.0`). The `hjust` and `vjust` parameters can also be pushed past 0 and 1 to better nudge around grob-columns to where the user wants them, though it should be noted that if they go past > 1 or < 0, the grob-columns will be outside of their allotted space.

```{r hjust and vjust}
gl = grob_layout(
  grob_row(
    # center
    grob_col(1, border = TRUE, padding = 5),
    # top, center
    grob_col(2, border = TRUE, padding = 5, vjust = 1),
    # extreme top, center
    grob_col(3, border = TRUE, padding = 5, vjust = 1.75)
    ),
  grob_row(
    # extreme bottom, left
    grob_col(4, border = TRUE, padding = 5, hjust = -0.5, vjust = -1),
    # bottom, left
    grob_col(5, border = TRUE, padding = 5, hjust = 0, vjust = 0),
    # top, left
    grob_col(6, border = TRUE, padding = 5, hjust = 0, vjust = 1)
    ),
  height = 100,
  width = 100,
  padding = 5
...
### Size Control - Proportion Parameter
...
gl = grob_layout(
  grob_row(grob_col('p=2'), p = 2, border = TRUE),
  grob_row(grob_col('p=1'), p = 1, border = TRUE),
  height = 100,
  width = 100,
  padding = 5
  )
...
There are two grob-rows on the same, outermost layer of the grob-layout, so the height of the page, 90 mm (removing 10 mm for padding), will be distributed between those two grob-rows. The `p` parameter (standing for "proportion"), controls how the height is proportionally distributed. Since the first grob-row has a `p` of 2, and the second has a `p` of 1, the first grob-row gets `2/(2+1) = 2/3` of the height (60 mm), and the second grob-row gets `1/(2+1) = 1/3` of the height (30 mm). If the user sets `p` to a positive value less than one, say `p = 1/3`, then that grob-row would receive one-third of the proportion of the grob-rows on its level/layer (assuming all other grob-rows on its level/layer have `p = 1`). The same logic applies to grob-columns:

```{r 1x2 system - p parameter}
gl = grob_layout(
  grob_row(
    grob_col('p=2', p = 2, border = TRUE),
    grob_col('p=1', p = 1, border = TRUE)
    ),
  height = 100,
  width = 100,
  padding = 5
...
In the `grobblR` package, there are many aspects the user can control the aesthetics of: Borders with the `border_aes_list` parameter, titles with the `title_aes_list` parameter, and aesthetics within the grobs themselves with the `aes_list` parameter. The user can only utilize these aesthetic parameters by using the `ga_list()` function, which has a wide variety of aesthetic options accepted by `grobblR` (such as `text_color` and `border_width`). Another pro of the function is that the user can easily see which aesthetic tweaks are allowed by going through the parameters of `ga_list()`. Examples of how `ga_list()` is implemented can be seen below in succeeding sections.

### Borders

As we just saw, setting `border = TRUE` in either `grob_row()` or `grob_col()` puts a border around the respective grob-row or grob-column. Those borders can be customized with the `border_aes_list` parameter:
...
If a `title` is inputted into any of `grob_col()`, `grob_row()` or `grob_layout()`, a title will appear at the top of the respective allotted space. The default is the title taking up 10% (`title_p = 0.1`) of the allotted height (the title can take up a maximum of 25%), and a `gray40` color background with white font. The `title_aes_list` parameter controls the aesthetics of the title grob. The title grob is treated as a single value `1 x 1` matrix, so the aesthetic parameters in `title_aes_list` are treated the same as described in the Matrix Grobs section later on. 
...
Data frames/matrices are probably the most complex of all the accepted grob objects in the `grobblR` package. With matrices, each element is essentially its own tiny grob. So if the user has a `2 x 2` matrix which he/she wants to convert to a grob, the `grobblR` method is to create 4 distinct tiny grobs and combine them all together to create the appearance of the matrix which was inputted. This allows for very customizable looks for matrices - which we needed to do a lot in Sacramento. If we needed to color specific elements of the matrix, or change their font face, or change their font size, we could do that using this methodology. 
...
These flexible aesthetics make for an incredibly customizable matrix look. Say the user was only concerned with the background color of a matrix, and he/she wanted to have it uniform for every cell, all the user would need to do is set a single value for `background_color` as an element within `aes_list`:
...
But say the user wanted different cells of the matrix to have different background colors. He/she would need to specify a separate aesthetic matrix corresponding to the background color of each cell of the original matrix. Since `mat` is a `2 x 2` matrix, we need to specify a another `2 x 2` matrix with each element denoting a background color for each of the cells in `mat`:

```{r First matrix, cell-by-cell customization, 1}

grob_matrix_object = mat %>%
  grob_matrix_object() %>%
  add_aesthetic(aesthetic = 'background_color', group = 'cells', value = 'white')
...
The same can be said for most of the other aesthetic elements listed above. For example, the `text_color` can be changed for individual elements of a matrix, but `cell_color_gradient_binary` is a `TRUE`/`FALSE` value, and cannot be changed for individual elements. Below is a grob-layout with 4 matrix grobs with various aesthetic elements tweaked.

```{r First matrix, cell-by-cell customization, 2}
mat_char = matrix(c('Apple', 'Pear', 'Grape', 'Cherry'), nrow = 2, byrow = TRUE)
...
gl = grob_layout(
  grob_row(
    grob_col(
      df,
      aes_list = ga_list(cell_color_gradient_columns = 3)
      ),
    grob_col(
      df,
      aes_list = ga_list(
        cell_color_gradient_columns = 3,
        cell_color_gradient_binary = TRUE,
        cell_color_binary_cut_off = 2.5
        )
      )
    ),
  grob_row(
    grob_col(
      df,
      aes_list = ga_list(
        cell_color_gradient_columns = c(2,3),
        cell_color_gradient_max = 'blue',
        cell_color_gradient_mid = 'purple',
        cell_color_gradient_min = 'red'
        )
      ),
    grob_col(
      df,
      aes_list = ga_list(
        cell_color_gradient_columns = c(2,3),
        cell_color_gradient_binary = TRUE,
        cell_color_binary_cut_off = 2,
        cell_color_binary_high = 'yellow',
        cell_color_binary_equal = 'green',
        cell_color_binary_low = 'blue'
        )
      )
    ),
  height = 100,
...
For every matrix aesthetic option (except the color gradient options), there will be an option to 
...
gl = grob_layout(
  grob_row(grob_col(plain_vector), border = TRUE),
  grob_row(grob_col(named_vector), border = TRUE),
  height = 100,
  width = 100
  )
...
The next objects accepted by `grobblR` are `ggplot2` plots. The `ggplot` is stretched or squished depending on what the dimensions of the allotted space are:
...
There are a couple things unique to character strings that the user can tweak in `aes_list` and `ga_list()`:  
  ...
  * `str_sep` - The separator within the character string which designates where a new line should start.
...
gl = grob_layout(
  grob_row(border = TRUE, grob_col(text, border = T)),
  grob_row(
    p = 2/3,
    border = TRUE,
    grob_col(
      border = TRUE,
      aes_list = ga_list(n_lines = 1, text_just = 'right'),
      text
      )
    ),
  grob_row(
    ...
    grob_col(
      aes_list = ga_list(
        ...
        text_just = 'left',
        text_color = 'purple',
        font_face = matrix(c(1,2,1,1,1), ncol = 1)
        ),
      bulleted_list
      )
    ),
...
It should be noted that the default values for the `height` and `width` parameters within `grob_layout()` are `280` and `216` and the default units are `mm`, as these are the values needed to properly fit the grob-layout onto a piece of standard computer paper, portrait orientation. (Other potential options for the) Thus, the defaults in `grob_layout()` are meant for printing and fitting the PDF of the grob-layout onto a piece of paper. Obviously there are other uses for PDF files, but this is the one we found most useful. Before demonstrating how to convert these grob-layouts to actual PDF files, let's create a couple:
...
player_caption_aes_list = ga_list(
  text_just = 'center'
  )

first_page_gl = grob_layout(
  title = '2001-02 Sacramento Kings',
  title_aes_list = main_title_aes_list,
  grob_row(
    grob_col('kings_logo_old.png', p = 1/2),
    grob_col(
      caption = 'COACH - Rick Adelman',
      caption_aes_list = player_caption_aes_list,
      'adelman.png'
      ),
    grob_col(
      p = 1/2,
  ...
  grob_row(
    ...
    grob_col(NA, p = 1/2),
    grob_col(
      caption = "PG - Mike Bibby",
      caption_aes_list = player_caption_aes_list,
      'bibby.png'
      ),
    grob_col(
      caption = 'SG - Doug Christie',
      caption_aes_list = player_caption_aes_list,
      'christie.png'
      ),
    grob_col(p = 1/2, NA)
  ...
  grob_row(
    ...
    title_aes_list = title_aes_list,
    grob_col(
      caption = 'SF - Peja Stojakovic',
      caption_aes_list = player_caption_aes_list,
      'stojakovic.png'
      ),
    grob_col(
      caption = 'C - Vlade Divac',
      caption_aes_list = player_caption_aes_list,
      'divac.png'
      ),
    grob_col(
      caption = 'PF - Chris Webber',
      caption_aes_list = player_caption_aes_list,
      'webber.png'
      )
    ),
...
second_page_gl = grob_layout(
  title = "2001-02 Season Accolades",
  title_aes_list = main_title_aes_list,
  grob_row(
    grob_col(
      p = 1/5,
      aes_list = ga_list(round_rect_radius = 0.2, background_color = 'gray40'),
      ""
      ),
    grob_col(
      caption = "BE - Hedo Turkoglu",
      caption_aes_list = player_caption_aes_list,
      'hedo.png'
      ),
  ...
  grob_row(
    grob_col(
      aes_list = ga_list(n_lines = 5, text_color = sac_purple, text_just = 'left'),
      bulleted_list
      ),
    grob_col(
      aes_list = ga_list(n_lines = 5, text_color = sac_purple, text_just = 'left'),
  ...
  grob_row(
    ...
    border_aes_list = ga_list(
      border_width = 4,
      border_color = sac_purple
      ),
    grob_col(
      caption = 'All-NBA, 2nd Team',
      caption_aes_list = ga_list(text_just = 'center', text_color = sac_purple),
      'webber.png'
      ),
    grob_col(
      caption = 'All-Defense, 2nd Team',
      caption_aes_list = ga_list(text_just = 'center', text_color = sac_purple),
      'christie.png'
      )
    )
...
grob_to_pdf(
  ...
  meta_data_title = 'grobblR SAC 2001-02 Summary'
  )
```
...
The `grobblR` package provides R users with a way to quickly create dynamic reports with attention to detail - a feature we did not find to be readily available in other packages. In the future we hope to enhance and add even more features, but, even as it currently stands, we believe `grobblR` is a valuable utility in daily data visualization and data reporting tasks.
